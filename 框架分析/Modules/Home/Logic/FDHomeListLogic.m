...
@implementation FDHomeListLogic
-(instancetype)init{
    self = [super init];
    if (self) {
        _dataArray = @[].mutableCopy;
        _dataArraySection = @[].mutableCopy;
    ...
    [HRHTTPTool postWithURL:kJRG_index_info parameters:nil success:^(id json) {
        ...
        if ([result intValue] == 200) {
            if ([json isKindOfClass:[NSDictionary class]]) {
                ...
                self.dataArraySection = [json objectForKey:@"result"];
                
                for (NSDictionary *dict in self.dataArraySection) {
                    NSArray *arrTemp = dict[@"data"];
                    [tempNewsArr addObject:arrTemp];
