...
- (void)viewDidLoad
{
    ...
    [self prepareData];
    self.cxSearchTextField.text = _textfieldText;
...
//    [self.cxSearchTextField addTarget:self action:@selector(textFieldChange:) forControlEvents:UIControlEventEditingChanged];
    self.searchBackgroundView.backgroundColor = [GFICommonTool colorWithHexString:@"#01476C"];
}
...
- (void)prepareData
{
     ...
     */
    NSDictionary *testDict = @{@"section_id":@"1",@"section_title":@"金融狗",@"section_content":@[]};//@{@"content_name":@"化妆棉"}
...
#pragma mark - ——————— 导航栏按钮事件 ————————
//搜索事件
- (IBAction)serachButtonAction:(id)sender {
    [self.cxSearchTextField resignFirstResponder];
    [[self presentingViewController] dismissViewControllerAnimated:YES completion:nil];
}
//返回事件
- (IBAction)backMainControllerAction:(id)sender {
    [self.cxSearchTextField resignFirstResponder];
    [[self presentingViewController] dismissViewControllerAnimated:YES completion:nil];
}
...
//    [al show];
    ...
    if ([self.delegate respondsToSelector:@selector(goToSearchView:)]) {
        [self.delegate goToSearchView:contentModel.content_name];
    }
...
//    self.cxSearchTextField.text = @"";
    ...
    if ([self.delegate respondsToSelector:@selector(goToSearchView:)]) {
        [self.delegate goToSearchView:textString];
    }
