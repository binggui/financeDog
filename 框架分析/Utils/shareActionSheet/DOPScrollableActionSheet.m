...
//        height += 60;
        /*calculation end*/
        self.frame = CGRectMake(0, _screenRect.size.height, _screenRect.size.width, 210);
        ...
        for (int i = 0; i < rowCount; i++) {
            if ([_actions[i] isKindOfClass:[NSString class]]) {
                //title
                    ...
                    UILabel *rowTitle = [[UILabel alloc] initWithFrame:CGRectMake(_screenRect.size.width/2 - 60, 30, 120, 20)];
                    rowTitle.font = [UIFont systemFontOfSize:18.0f];
                    rowTitle.text = _actions[i];
                    rowTitle.textColor = [UIColor blackColor];
                    rowTitle.textAlignment = NSTextAlignmentCenter;
//                    rowTitle.backgroundColor = RGBACOLOR(255,92,49,1);
                    rowTitle.backgroundColor = [UIColor whiteColor];
                    UIView *lineView = [[UIView alloc]initWithFrame:CGRectMake(0, 40, KScreenWidth, 1)];
                    lineView.backgroundColor = [GFICommonTool colorWithHexString:@"#dcdcdc"];
                ...
                //actions array
                UIScrollView *rowContainer = [[UIScrollView alloc] initWithFrame:CGRectMake(0, y, _screenRect.size.width, 180)];
                ...
                rowContainer.contentSize = CGSizeMake(items.count*80+20, 180);
                rowContainer.backgroundColor = [UIColor whiteColor];
                [self addSubview:rowContainer];
                //add each item
                CGFloat x = (_screenRect.size.width - 120 - 100)/2;
                for (int j = 0; j < items.count; j++) {
                    DOPAction *action = items[j];
                    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
                    button.frame = CGRectMake(x, 30, 60, 60);
                    ...
                    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(x - 10, 90, 80, 30)];
...
- (void)show {
    ...
    [UIView animateWithDuration:0.2 animations:^{
        self.dimBackground.backgroundColor = [UIColor colorWithWhite:0.0 alpha:0.3];
